...
def session_storage(temp_dir):
    ...
    with patch.object(SessionStorage, '_get_config_directory', return_value=temp_dir / "mathsfun"):
...
class TestSessionStorageInit:
    ...
    def test_init_custom_app_name(self, temp_dir):
        ...
        with patch.object(SessionStorage, '_get_config_directory', return_value=temp_dir / "custom_app"):
    ...
    def test_init_creates_config_directory(self, temp_dir):
        ...
        with patch.object(SessionStorage, '_get_config_directory', return_value=config_dir):
        ...
        with patch.object(SessionStorage, '_get_config_directory', return_value=config_dir):
            with patch.object(Path, 'mkdir', side_effect=PermissionError("Access denied")):
                with patch('builtins.print') as mock_print:
                    ...
                    mock_print.assert_called_with("Warning: Could not create config directory: Access denied")
...
class TestSaveSession:
    ...
    @patch('os.name', 'posix')
    @patch('os.chmod')
    def test_save_session_sets_file_permissions_unix(self, mock_chmod, session_storage, sample_session_data):
        ...
        expected_calls = [
            ...
            unittest.mock.call(session_storage._session_file.with_suffix('.tmp'), stat.S_IRUSR | stat.S_IWUSR)
        ]
        # Check that chmod was called for the temp file with correct permissions
        mock_chmod.assert_any_call(session_storage._session_file.with_suffix('.tmp'), stat.S_IRUSR | stat.S_IWUSR)

    @patch('os.name', 'nt')
    @patch('os.chmod')
    def test_save_session_no_file_permissions_windows(self, mock_chmod, session_storage, sample_session_data):
    ...
    def test_save_session_atomic_operation(self, session_storage, sample_session_data):
        ...
        with patch('pathlib.Path.rename') as mock_rename:
            ...
            assert session_storage._session_file.exists()
            mock_print.assert_called_with("Warning: Could not save session: Rename failed")

    def test_save_session_cleanup_on_failure(self, session_storage, sample_session_data):
    ...
    def test_save_session_cleanup_exception_handling(self, session_storage, sample_session_data):
...
class TestLoadSession:
    ...
    @patch('os.name', 'posix')
    def test_load_session_insecure_permissions(self, session_storage, sample_session_data):
        ...
        with patch.object(Path, 'stat') as mock_stat:
            ...
            mock_print.assert_called_with("Warning: Session file has insecure permissions, removing it")

    @patch('os.name', 'nt')
    def test_load_session_skip_permissions_check_windows(self, session_storage, sample_session_data):
...
class TestClearSession:
    ...
    def test_clear_session_exception_handling(self, session_storage, sample_session_data):
        ...
        assert result is False
        mock_print.assert_called_with("Warning: Could not clear session file: Cannot delete")
...
class TestGetSessionInfo:
    ...
    def test_get_session_info_expired_session(self, session_storage, expired_session_data):
...
class TestSessionStorageIntegration:
    ...
    def test_concurrent_access_simulation(self, session_storage, sample_session_data):
        ...
        loaded_data = session_storage.load_session()
        assert "new" in str(loaded_data) or loaded_data == sample_session_data  # Either new data succeeded or old data preserved
