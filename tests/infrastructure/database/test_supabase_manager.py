...
def mock_session_storage():
    ...
    with patch('src.infrastructure.database.supabase_manager.SessionStorage') as mock_storage_class:
        mock_storage = Mock()
...
def mock_supabase_client():
    ...
    with patch('src.infrastructure.database.supabase_manager.create_client') as mock_create:
        mock_client = Mock()
...
def supabase_manager(mock_session_storage, mock_supabase_client):
    ...
    with patch.dict('src.infrastructure.database.supabase_manager.__dict__', {'url': 'http://test.supabase.co', 'key': 'test-key'}):
...
class TestStartOAuthServer:
    ...
    def test_start_oauth_server(self, mock_sleep, mock_thread, mock_oauth_server):
        ...
        with patch('src.infrastructure.database.supabase_manager.threading.Event') as mock_event:
        ...
        mock_oauth_server.assert_called_once_with(("localhost", 8080), OAuthCallbackHandler)
...
class TestSupabaseManagerBasicMethods:
    ...
    def test_sign_out(self, supabase_manager, mock_session_storage, mock_supabase_client):
...
class TestSupabaseManagerSessionManagement:
    ...
    def test_save_session_success(self, supabase_manager, mock_session_storage):
        ...
        assert result is True
        mock_session_storage.save_session.assert_called_once_with({"access_token": "test_token"})
    ...
    def test_restore_session_success_set_session(self, supabase_manager, mock_supabase_client):
    ...
    def test_restore_session_fallback_to_refresh(self, supabase_manager, mock_supabase_client):
        ...
        mock_supabase_client.auth.set_session.side_effect = Exception("Set session failed")
        ...
        assert result is True
        mock_supabase_client.auth.refresh_session.assert_called_once_with("test_refresh_token")

    def test_restore_session_refresh_fails(self, supabase_manager, mock_supabase_client):
    ...
    def test_restore_session_exception(self, supabase_manager, mock_supabase_client):
        ...
        mock_supabase_client.auth.refresh_session.side_effect = Exception("Refresh error")
        
        result = supabase_manager.restore_session(session_data)
        assert result is False

    def test_load_persisted_session_no_data(self, supabase_manager, mock_session_storage):
    ...
    def test_load_persisted_session_success(self, supabase_manager, mock_session_storage):
        ...
        with patch.object(supabase_manager, 'restore_session', return_value=True) as mock_restore:
            result = supabase_manager.load_persisted_session()
        
        assert result is True
        mock_restore.assert_called_once_with(session_data)

    def test_load_persisted_session_restore_fails(self, supabase_manager, mock_session_storage):
        ...
        with patch.object(supabase_manager, 'restore_session', return_value=False) as mock_restore:
            result = supabase_manager.load_persisted_session()
        
        assert result is False
        mock_session_storage.clear_session.assert_called_once()

    def test_load_persisted_session_exception(self, supabase_manager, mock_session_storage):
        ...
        assert result is False
        mock_print.assert_called_once_with("Warning: Error loading persisted session: Load error")
...
class TestSignInWithGoogle:
    ...
    def test_sign_in_with_google_no_oauth_url(self, mock_time, mock_sleep, mock_create_client, mock_start_server, mock_webbrowser, supabase_manager):
    ...
    def test_sign_in_with_google_timeout(self, mock_print, mock_time, mock_sleep, mock_create_client, mock_start_server, mock_webbrowser, supabase_manager):
    ...
    @patch('src.infrastructure.database.supabase_manager.time.time')
    @patch('builtins.print')
    def test_sign_in_with_google_server_error_result(self, mock_print, mock_time, mock_sleep, mock_create_client, mock_start_server, mock_webbrowser, supabase_manager):
    ...
    @patch('src.infrastructure.database.supabase_manager.start_oauth_server')
    @patch('src.infrastructure.database.supabase_manager.create_client')
    def test_sign_in_with_google_general_exception(self, mock_create_client, mock_start_server, mock_webbrowser, supabase_manager):
...
class TestValidateEnvironment:
    ...
    def test_validate_environment_missing_url(self):
        ...
        with patch.dict('src.infrastructure.database.supabase_manager.__dict__', {'url': '', 'key': 'test-key'}):
            valid, message = validate_environment()
            
            assert valid is False
            assert "Missing SUPABASE_URL or SUPABASE_KEY" in message

    def test_validate_environment_missing_key(self):
        """Test validate_environment with missing key."""
        with patch.dict('src.infrastructure.database.supabase_manager.__dict__', {'url': 'http://test.supabase.co', 'key': ''}):
            valid, message = validate_environment()
...
class TestPKCEStorage:
    ...
    def test_pkce_storage_operations(self, supabase_manager):
        ...
        with patch('src.infrastructure.database.supabase_manager.start_oauth_server') as mock_start_server:
            mock_server = Mock()
            mock_server.auth_result = {"success": False}  # To exit early
            mock_start_server.return_value = mock_server
            
            with patch('src.infrastructure.database.supabase_manager.create_client') as mock_create_client:
                mock_oauth_client = Mock()
                mock_oauth_response = Mock()
                mock_oauth_response.url = "https://oauth.example.com"
                mock_oauth_client.auth.sign_in_with_oauth.return_value = mock_oauth_response
                ...
                with patch('src.infrastructure.database.supabase_manager.time.time', side_effect=[0, 1, 2]):
