...
class TestCreateSession:
    ...
    def test_create_session_success(self, quiz_repository, mock_client, sample_quiz_session, sample_session_dict):
    ...
    def test_create_session_exception(self, quiz_repository, mock_client, sample_quiz_session):
    ...
    def test_create_session_no_data_returned(self, quiz_repository, mock_client, sample_quiz_session):
...
class TestGetSession:
    ...
    def test_get_session_success(self, quiz_repository, mock_client, sample_session_dict):
        ...
        mock_client.table.return_value.select.return_value.eq.assert_called_once_with("id", "session-123")
...
class TestUpdateSession:
    ...
    def test_update_session_success(self, quiz_repository, mock_client, sample_quiz_session, sample_session_dict):
        ...
        mock_client.table.return_value.update.return_value.eq.assert_called_once_with("id", "session-123")
    ...
    def test_update_session_exception(self, quiz_repository, mock_client, sample_quiz_session):
    ...
    def test_update_session_no_data_returned(self, quiz_repository, mock_client, sample_quiz_session):
...
class TestCompleteSession:
    ...
    def test_complete_session_not_authenticated(self, mock_supabase_manager, mock_client):
...
class TestGetUserSessions:
    ...
    def test_get_user_sessions_success(self, quiz_repository, mock_client, sample_session_dict):
        ...
        mock_client.table.assert_called_once_with("quiz_sessions")
        mock_client.table.return_value.select.assert_called_once_with("*")
        mock_client.table.return_value.select.return_value.eq.assert_called_once_with("user_id", "user-456")
        mock_client.table.return_value.select.return_value.eq.return_value.order.assert_called_once_with("start_time", desc=True)
        mock_client.table.return_value.select.return_value.eq.return_value.order.return_value.limit.assert_called_once_with(50)
    ...
    def test_get_user_sessions_custom_limit(self, quiz_repository, mock_client, sample_session_dict):
        ...
        mock_client.table.return_value.select.return_value.eq.return_value.order.return_value.limit.assert_called_once_with(25)
    ...
    def test_get_user_sessions_not_authenticated(self, mock_supabase_manager, mock_client):
    ...
    def test_get_user_sessions_exception(self, quiz_repository, mock_client):
        ...
        mock_client.table.return_value.select.return_value.eq.return_value.order.return_value.limit.return_value.execute.side_effect = Exception("DB Error")
...
class TestSaveAttempt:
    ...
    def test_save_attempt_success(self, quiz_repository, mock_client, sample_problem_attempt, sample_attempt_dict):
    ...
    def test_save_attempt_exception(self, quiz_repository, mock_client, sample_problem_attempt):
    ...
    def test_save_attempt_no_data_returned(self, quiz_repository, mock_client, sample_problem_attempt):
...
class TestGetSessionAttempts:
    ...
    def test_get_session_attempts_success(self, quiz_repository, mock_client, sample_attempt_dict):
        ...
        mock_client.table.assert_called_once_with("problem_attempts")
        mock_client.table.return_value.select.assert_called_once_with("*")
        mock_client.table.return_value.select.return_value.eq.assert_called_once_with("session_id", "session-456")
        mock_client.table.return_value.select.return_value.eq.return_value.order.assert_called_once_with("timestamp")
    ...
    def test_get_session_attempts_not_authenticated(self, mock_supabase_manager, mock_client):
...
class TestIncrementSessionStats:
    ...
    def test_increment_session_stats_success_correct(self, quiz_repository, mock_client, sample_quiz_session, sample_session_dict):
        """Test successful stats increment for correct answer."""
        # Mock get_session to return existing session
        with patch.object(quiz_repository, 'get_session', return_value=sample_quiz_session):
            mock_response = Mock()
            mock_response.data = [sample_session_dict]
            mock_client.table.return_value.update.return_value.eq.return_value.execute.return_value = mock_response
            
            result = quiz_repository.increment_session_stats("session-123", is_correct=True)
        ...
        update_call.assert_called_once_with({
            "total_problems": 11,  # 10 + 1
            "correct_answers": 9   # 8 + 1
        })

    def test_increment_session_stats_success_incorrect(self, quiz_repository, mock_client, sample_quiz_session, sample_session_dict):
        ...
        with patch.object(quiz_repository, 'get_session', return_value=sample_quiz_session):
            ...
            result = quiz_repository.increment_session_stats("session-123", is_correct=False)
        ...
        update_call.assert_called_once_with({
            "total_problems": 11,  # 10 + 1
            "correct_answers": 8   # 8 + 0
        })

    def test_increment_session_stats_session_not_found(self, quiz_repository, mock_client):
        """Test increment_session_stats when session is not found."""
        with patch.object(quiz_repository, 'get_session', return_value=None):
            result = quiz_repository.increment_session_stats("nonexistent", is_correct=True)
        
        assert result is False
        # Should not make update call
        mock_client.table.assert_not_called()

    def test_increment_session_stats_not_authenticated(self, mock_supabase_manager, mock_client):
    ...
    def test_increment_session_stats_exception(self, quiz_repository, mock_client, sample_quiz_session):
    ...
    def test_increment_session_stats_update_returns_none(self, quiz_repository, mock_client, sample_quiz_session):
...
class TestQuizRepositoryIntegration:
    ...
    def test_all_methods_require_authentication(self):
        ...
        methods = [method for method in dir(QuizRepository) 
                  if not method.startswith('_') 
                  and callable(getattr(QuizRepository, method))]
        
        # Filter out inherited methods from BaseRepository
        quiz_specific_methods = [
            'create_session', 'get_session', 'update_session', 
            'complete_session', 'get_user_sessions', 'save_attempt', 
            'get_session_attempts', 'increment_session_stats'
        ]
        ...
        for method_name in quiz_specific_methods:
            ...
            assert hasattr(method, '__wrapped__') or 'requires_authentication' in str(method)