...
class TestUserRepositoryDataHandling:
    ...
    def test_create_user_profile_with_minimal_data(
        self, minimal_user, minimal_db_response
    ):
...
class TestUserRepositoryAuthentication:
    ...
    def test_methods_blocked_when_not_authenticated(self, sample_user):
        """Test that repository methods return None when user is not authenticated."""
        # Setup unauthenticated SupabaseManager
        mock_supabase_manager = Mock()
        mock_supabase_manager.is_authenticated.return_value = False

        repository = UserRepository(mock_supabase_manager)

        # Execute and verify all methods return None when not authenticated
        assert repository.get_user_profile("test-user-123") is None
        assert repository.create_user_profile(sample_user) is None
        assert repository.update_user_profile(sample_user) is None
        assert repository.update_last_active("test-user-123") is None

        # Verify that get_client was never called
        mock_supabase_manager.get_client.assert_not_called()

    def test_authentication_check_message(self, capsys):
        """Test that proper authentication error message is printed."""
        # Setup unauthenticated SupabaseManager
        mock_supabase_manager = Mock()
        mock_supabase_manager.is_authenticated.return_value = False
        ...
        # Verify result and message
        assert result is None
        captured = capsys.readouterr()
        assert "User not authenticated" in captured.out
