...
import pytest
from src.presentation.controllers.session import format_duration, show_results, prompt_start_session
...
class TestShowResults:
    ...
    @pytest.mark.parametrize("correct,total,expected_message", [
    ...
    def test_accuracy_messages(self, capsys, mock_generator, correct, total, expected_message):
...
class TestPromptStartSession:
    ...
    def test_prompt_limited_session(self, capsys, mocker, mock_generator):
        ...
        assert f"✅ Ready to start! {mock_generator.num_problems} problems prepared." in output
        assert "Press Enter when you're ready to start" in output
        mock_input.assert_called_once()