...
class TestGetTableRange:
    ...
    def test_get_table_range_low_too_high(self, mock_print, mock_get_user_input):
        ...
        mock_get_user_input.side_effect = ["101", "5", "10"]  # First invalid, then valid
    ...
    def test_get_table_range_high_out_of_bounds(self, mock_print, mock_get_user_input):
        ...
        mock_get_user_input.side_effect = ["5", "101", "5", "10"]  # Low, invalid high, low again, valid high
    ...
    @patch('src.presentation.controllers.addition_tables.get_user_input')
    @patch('builtins.print')
    def test_get_table_range_low_greater_than_high(self, mock_print, mock_get_user_input):
        """Test when low number is greater than high number."""
        mock_get_user_input.side_effect = ["10", "5", "3", "8"]  # First pair invalid, second valid
        
        result = get_table_range()
        
        assert result == (3, 8)
        mock_print.assert_called_with("❌ Low number must be less than or equal to high number")

    @patch('src.presentation.controllers.addition_tables.get_user_input')
    @patch('builtins.print')
    def test_get_table_range_invalid_number_format(self, mock_print, mock_get_user_input):
        """Test invalid number format input."""
        mock_get_user_input.side_effect = ["abc", "5", "10"]  # First invalid, then valid
    ...
    @patch('src.presentation.controllers.addition_tables.get_user_input')
    @patch('builtins.print')
    def test_get_table_range_multiple_validation_failures(self, mock_print, mock_get_user_input):
        """Test multiple validation failures before success."""
        mock_get_user_input.side_effect = [
            "0",      # Invalid: too low
            "101",    # Invalid: too high  
            "10", "5",  # Invalid: low > high
            "2", "7"    # Valid
        ]
...
class TestRunAdditionTableQuiz:
    ...
    def test_run_quiz_complete_all_problems(self, mock_time, mock_print, mock_input):
        ...
        mock_input.side_effect = ["", "2"]  # Empty string for "Press Enter", then answer
    ...
    @patch('builtins.print')
    @patch('time.time')
    def test_run_quiz_wrong_then_correct_answer(self, mock_time, mock_print, mock_input):
    ...
    def test_run_quiz_mixed_results(self, mock_time, mock_print, mock_input):
        ...
        assert correct == 3  # First, second (after retry), and fourth correct
        assert total == 4  # Four actual attempts (skip doesn't count as attempt)
        assert duration == 15
...
class TestAdditionTablesMode:
    ...
    @patch('src.presentation.controllers.addition_tables.get_table_range')
    @patch('builtins.print')
    def test_addition_tables_mode_single_number(self, mock_print, mock_get_range, mock_get_order, mock_run_quiz, mock_show_results):
        ...
        mock_run_quiz.return_value = (1, 1, 30.0)
    ...
    @patch('src.presentation.controllers.addition_tables.get_order_preference')
    @patch('src.presentation.controllers.addition_tables.get_table_range')
    def test_addition_tables_mode_generator_created_correctly(self, mock_get_range, mock_get_order, mock_run_quiz, mock_show_results):
        ...
        mock_run_quiz.return_value = (0, 0, 0)
        
        with patch('src.presentation.controllers.addition_tables.AdditionTableGenerator') as mock_generator_class:
            ...
            # Verify generator was passed to show_results
            mock_show_results.assert_called_once_with(0, 0, 0, mock_generator_instance)
