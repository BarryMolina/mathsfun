...
class AnalyticsState:
    ...
    def __init__(self, all_performances: List["MathFactPerformance"], 
                 math_fact_service: "MathFactService", user_id: str):
    ...
    def refresh_data(self):
        ...
        self.original_performances = self.math_fact_service.get_all_user_performances(self.user_id)
        self.filtered_performances = self.original_performances.copy()
        self.active_filters = []
    ...
    def apply_sort(self, field: str, descending: bool = True):
        ...
        try:
            if field == "fact":
                self.filtered_performances.sort(key=lambda p: p.fact_key, reverse=descending)
            elif field == "attempts":
                self.filtered_performances.sort(key=lambda p: p.total_attempts, reverse=descending)
            elif field == "accuracy":
                self.filtered_performances.sort(key=lambda p: p.accuracy, reverse=descending)
            elif field == "time":
                self.filtered_performances.sort(key=lambda p: p.average_response_time_seconds, reverse=descending)
            elif field == "ease":
                self.filtered_performances.sort(key=lambda p: float(p.easiness_factor), reverse=descending)
            elif field == "interval":
                self.filtered_performances.sort(key=lambda p: p.interval_days, reverse=descending)
            elif field == "repetitions":
                self.filtered_performances.sort(key=lambda p: p.repetition_number, reverse=descending)
            elif field == "fastest":
                self.filtered_performances.sort(key=lambda p: p.fastest_response_ms or 99999, reverse=descending)
            elif field == "slowest":
                self.filtered_performances.sort(key=lambda p: p.slowest_response_ms or 0, reverse=descending)
            else:
    ...
    def apply_filter(self, condition: str):
        ...
        try:
            ...
            if condition == "due":
                filtered = [p for p in self.filtered_performances if p.is_due_for_review]
                filter_desc = "facts due for review"
            elif condition == "overdue":
                now = datetime.now(timezone.utc)
                filtered = [p for p in self.filtered_performances 
                           if p.next_review_date and p.next_review_date < now]
                filter_desc = "overdue facts"
            elif condition == "easy":
                filtered = [p for p in self.filtered_performances if float(p.easiness_factor) > 2.5]
                filter_desc = "easy facts (ease factor > 2.5)"
            elif condition == "difficult":
                filtered = [p for p in self.filtered_performances if float(p.easiness_factor) < 2.0]
                filter_desc = "difficult facts (ease factor < 2.0)"
            elif condition == "daily":
                filtered = [p for p in self.filtered_performances if p.interval_days == 1]
                filter_desc = "facts on daily review"
            elif condition == "new":
                filtered = [p for p in self.filtered_performances if p.repetition_number < 3]
                filter_desc = "new facts (repetitions < 3)"
            elif condition == "established":
                filtered = [p for p in self.filtered_performances if p.repetition_number >= 6]
                filter_desc = "established facts (repetitions ≥ 6)"
            else:
                # Parse numeric filters like "accuracy <80" or "ease >2.5"
                filtered, filter_desc = self._parse_numeric_filter(condition)
                if filtered is None:
    ...
    def _parse_numeric_filter(self, condition: str) -> Tuple[Optional[List], Optional[str]]:
...
def display_analytics_overview(state: AnalyticsState) -> None:
    ...
    print(f"Total facts tracked: {total_facts} | Due for review: {facts_due} | Average ease factor: {avg_ease:.1f} | Overall accuracy: {total_accuracy:.1f}%")
...
def display_analytics_table(state: AnalyticsState) -> None:
    ...
    # Table header
    print("Fact | Attempts | Accuracy | Avg Time | Ease Factor | Interval | Next Review | Repetitions | Fastest | Slowest")
    print("-" * 100)
    ...
    for p in performances:
        ...
        fastest = f"{(p.fastest_response_ms or 0) / 1000:.1f}s".rjust(7) if p.fastest_response_ms else "N/A".rjust(7)
        slowest = f"{(p.slowest_response_ms or 0) / 1000:.1f}s".rjust(7) if p.slowest_response_ms else "N/A".rjust(7)
        
        print(f"{fact} | {attempts} | {accuracy} | {avg_time} | {ease_factor} | {interval} | {next_review} | {repetitions} | {fastest} | {slowest}")
...
def show_analytics_help() -> None:
    ...
    print("  Fields: fact, attempts, accuracy, time, ease, interval, repetitions, fastest, slowest")
    print("  Examples: 'sort accuracy desc', 'sort time asc'")
    ...
    print("    Examples: 'filter accuracy <80', 'filter ease >2.5', 'filter attempts >=10'")
