...
class QuizService:
    ...
    def start_quiz_session(
        self, user_id: str, quiz_type: str, difficulty_level: int
    ...
    def complete_session(self, session_id: str) -> Optional[QuizSessionResult]:
        ...
        if attempts:
            ...
            correct_times = [
                attempt.response_time_ms for attempt in attempts if attempt.is_correct
            ]
    ...
    def get_user_progress(self, user_id: str, limit: int = 10) -> UserProgress:
        ...
        overall_accuracy = (
            (total_correct / total_problems * 100) if total_problems > 0 else 0.0
        )
        ...
        average_session_time = (
            sum(session_times) / len(session_times) if session_times else 0.0
        )

        # Find best accuracy
        best_accuracy = (
            max(session.accuracy for session in completed_sessions)
